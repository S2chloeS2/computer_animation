#
# Copyright (c) 2026. Columbia University. All rights reserved.
#
# This software and documentation contain confidential and proprietary
# information that is the property of Columbia University.
#
# Unauthorized copying, distribution, or modification of this file,
# via any medium, is strictly prohibited.
#
# Written by Changxi Zheng <cxz@cs.columbia.edu>, 2026
#
import fcl
import numpy as np
from scipy.spatial.transform import Rotation as R
from trimesh import Trimesh, inertia

from ..core.types import (
    Axis,
    AxisType,
    Vec3,
    Vec4,
)
from ..geometry.types import GeoType, ParticleFlags, ShapeFlags
from .model import Model


class ModelBuilder:
    """A helper class for building simulation models at runtime.

    Similar to many popular simulation engine (such as Newton, Drake, Mujoco),
    Use the ModelBuilder to construct a simulation scene. The ModelBuilder
    represents the scene using standard Python data structures like lists,
    which are convenient but unsuitable for efficient simulation.
    Call :meth:`finalize` to construct a simulation-ready Model.
    """

    # -------------------------------------------------------------------------
    # Default parameters

    # Default particle settings
    DEFAULT_PARTICLE_RADIUS = 0.1
    # Default restitution coefficients
    DEFAULT_PARTICLE_RESTITUTION_COEFF = 0.5

    DEFAULT_SHAPE_RESTITUTION_COEFF = 0.1
    DEFAULT_SHAPE_PENALTY_STIFFNESS = 1000.0
    DEFAULT_SHAPE_PENALTY_DAMPING = 0.0

    # Default cloth material parameters
    DEFAULT_CLOTH_STRETCH_STIFFNESS = 10.0
    DEFAULT_CLOTH_STRETCH_DAMPING = 0.1
    DEFAULT_CLOTH_SHEAR_STIFFNESS = 10.0
    DEFAULT_CLOTH_SHEAR_DAMPING = 0.1
    # -------------------------------------------------------------------------

    def __init__(self, up_axis: AxisType = Axis.Z, gravity: float = -9.81):

        self.up_axis: Axis = Axis.from_any(up_axis)
        self.gravity: float = gravity

        # particles
        self.particle_q = []
        self.particle_qd = []
        self.particle_mass = []
        self.particle_radius = []
        self.particle_flags = []
        self.particle_drag = []
        self.particle_restitution_coeff = []
        self.particle_uv = []

        # springs
        self.spring_indices = []
        self.spring_rest_length = []
        self.spring_stiffness = []
        self.spring_damping = []

        # gravitational
        self.gravitational_pairs = []
        self.gravitational_constant = []

        # shapes
        self.shape_type = []  # list[GeoType]
        self.shape_flags = []  # list[ShapeFlags]
        self.shape_transform = []  # nparray (shape_count, 4x4)
        self.shape_inv_transform = []  # nparray (shape_count, 4x4)
        self.shape_penalty_params = []  # nparray (shape_count, 2), float
        self.shape_restitution_coeff = []  # nparray (shape_count,), float
        self.shape_mass = []  # nparray (shape_count,), float
        self.shape_inv_mass = []  # nparray (shape_count,), float
        self.shape_inertia = []  # nparray (shape_count, 3x3), float
        self.shape_inv_inertia = []  # nparray (shape_count, 3x3), float
        # list[Any] (shape_count), typically a fcl.shape (e.g., fcl.Sphere, fcl.Box, fcl.Halfspace)
        self.shape_fcl_geoms = []
        self.shape_meshes = []

        self.tri_indices = []
        self.tri_areas = []
        self.tri_materials = []

        self.edge_indices = []
        self.edge_rest_angle = []
        self.edge_materials = []

    @property
    def particle_count(self) -> int:
        """
        The number of particles in the model.
        """
        return len(self.particle_q)

    @property
    def spring_count(self) -> int:
        """
        The number of springs in the model.
        """
        return len(self.spring_rest_length)

    # particles
    def add_particle(
        self,
        pos: Vec3,
        vel: Vec3,
        mass: float,
        radius: float | None = None,
        drag: float | None = None,
        flags: int = ParticleFlags.ACTIVE.value,
        restitution_coeff: float | None = None,
    ) -> int:
        """Adds a single particle to the model.

        Args:
            pos: The initial position of the particle.
            vel: The initial velocity of the particle.
            mass: The mass of the particle.
            radius: The radius of the particle used in collision handling. If None, the radius is set to the default
                    value (:attr:`DEFAULT_PARTICLE_RADIUS`).
            flags: The flags that control the dynamical behavior of the particle, see PARTICLE_FLAG_* constants.

        Note:
            Set the mass equal to zero to create a 'kinematic' particle that is not subject to dynamics.

        Returns:
            The index of the particle in the system.
        """
        if (isinstance(pos, list) and len(pos) != 3) or (isinstance(pos, np.ndarray) and pos.shape != (3,)):
            raise RuntimeError("pos must have a length of 3")
        if (isinstance(vel, list) and len(vel) != 3) or (isinstance(vel, np.ndarray) and vel.shape != (3,)):
            raise RuntimeError("vel must have a length of 3")
        self.particle_q.append(pos)
        self.particle_qd.append(vel)
        self.particle_mass.append(mass)
        self.particle_flags.append(flags)
        if radius is None:
            self.particle_radius.append(self.DEFAULT_PARTICLE_RADIUS)
        else:
            self.particle_radius.append(radius)
        if drag is None:
            self.particle_drag.append(0.0)
        else:
            self.particle_drag.append(drag)
        if restitution_coeff is None:
            self.particle_restitution_coeff.append(self.DEFAULT_PARTICLE_RESTITUTION_COEFF)
        else:
            self.particle_restitution_coeff.append(restitution_coeff)
        self.particle_uv.extend([[0.0, 0.0]])
        return self.particle_count - 1

    def add_particles(
        self,
        pos: list[Vec3],
        vel: list[Vec3],
        mass: list[float],
        radius: list[float] | None = None,
        drag: list[float] | None = None,
        flags: list[int] | None = None,
        restitution_coeff: list[float] | None = None,
    ) -> None:
        """Adds a group particles to the model.

        Args:
            pos: The initial positions of the particle.
            vel: The initial velocities of the particle.
            mass: The mass of the particles.
            radius: The radius of the particles used in collision handling. If None, the radius is set to the default
            value (:attr:`DEFAULT_PARTICLE_RADIUS`).
            flags: The flags that control the dynamical behavior of the particles, see PARTICLE_FLAG_* constants.

        Note:
            Set the mass equal to zero to create a 'kinematic' particle that is not subject to dynamics.
        """
        # check data
        for p, v in zip(pos, vel, strict=True):
            if (isinstance(p, list) and len(p) != 3) or (isinstance(p, np.ndarray) and p.shape != (3,)):
                raise RuntimeError("pos must have a length of 3")
            if (isinstance(v, list) and len(v) != 3) or (isinstance(v, np.ndarray) and v.shape != (3,)):
                raise RuntimeError("vel must have a length of 3")

        self.particle_q.extend(pos)
        self.particle_qd.extend(vel)
        self.particle_mass.extend(mass)
        if radius is None:
            radius = [self.DEFAULT_PARTICLE_RADIUS] * len(pos)
        if drag is None:
            drag = [0.0] * len(pos)
        if flags is None:
            flags = [ParticleFlags.ACTIVE.value] * len(pos)
        if restitution_coeff is None:
            restitution_coeff = [self.DEFAULT_PARTICLE_RESTITUTION_COEFF] * len(pos)
        self.particle_radius.extend(radius)
        self.particle_flags.extend(flags)
        self.particle_drag.extend(drag)
        self.particle_restitution_coeff.extend(restitution_coeff)
        self.particle_uv.extend([[0.0, 0.0]] * len(pos))

    def add_spring(self, i: int, j, ke: float, kd: float | None = None, rest_length: float | None = None):
        """Adds a spring between two particles in the system

        Args:
            i: The index of the first particle
            j: The index of the second particle
            ke: The elastic stiffness of the spring
            kd: The damping stiffness of the spring
            rest_length: The actuation level of the spring

        Note:
            If kd is None, zero damping will be used.

            If rest_length is None, the spring is created with a rest-length
            based on the distance between the particles in their initial
            configuration.
        """
        self.spring_indices.append(i)
        self.spring_indices.append(j)
        self.spring_stiffness.append(ke)
        self.spring_damping.append(0.0 if kd is None else kd)
        if rest_length is not None:
            if rest_length < 0:
                raise RuntimeError(f"Spring rest-length ({rest_length}) can't be negative")
            self.spring_rest_length.append(rest_length)
        else:
            self.spring_rest_length.append(-1.0)

    def add_shape_mesh(
        self,
        mesh: Trimesh,
        pos: Vec3,
        orient: Vec4,
        stiffness: float | None = None,
        damping: float | None = None,
        restitution_coeff: float | None = None,
        fixed: bool = False,
        density: float = 1.0,
    ):
        """Adds a mesh shape to the model.

        Args:
            mesh: The mesh of the shape
            pos: The position of the shape
            orient: The orientation of the shape
            stiffness: The stiffness of the shape
            damping: The damping of the shape
            restitution_coeff: The restitution coefficient of the shape
            fixed: Whether the shape is fixed
            density: The density of the shape
        NOTE:
            The orientation is given in quaternion (w, x, y, z) format (scalar first).
            When quaternion is [1, 0, 0, 0], the cylinder is aligned with the z axis.
        """
        self.shape_type.append(GeoType.MESH)
        if not isinstance(pos, np.ndarray):
            pos = np.asarray(pos, dtype=np.float64)
        if pos.shape != (3,):
            raise RuntimeError("pos must have a shape of (3,)")

        if not isinstance(orient, np.ndarray):
            orient = np.asarray(orient, dtype=np.float64)
        if orient.shape != (4,):
            raise RuntimeError("orient must have a shape of (4,)")
        ll = np.linalg.norm(orient)  # normalize the orientation
        if ll < 1e-8:
            raise RuntimeError("cylinder orient is too small, must be non-zero")
        orient /= ll
        self.shape_flags.append(ShapeFlags.ACTIVE.value if not fixed else 0)
        # make sure center of mass is at the origin of local frame
        mesh.vertices -= mesh.center_mass

        trans = np.eye(4)
        trans[:3, :3] = R.from_quat(orient, scalar_first=True).as_matrix()
        trans[:3, 3] = pos
        self.shape_transform.append(trans)
        self.shape_inv_transform.append(np.linalg.inv(trans))

        if stiffness is None:
            stiffness = self.DEFAULT_SHAPE_PENALTY_STIFFNESS
        elif stiffness <= 0.0:
            raise RuntimeError(f"Invalid stiffness ({stiffness} must > 0)")
        if damping is None:
            damping = self.DEFAULT_SHAPE_PENALTY_DAMPING
        elif damping < 0.0:
            raise RuntimeError(f"Invalid damping ({damping} must >= 0)")

        self.shape_penalty_params.append([stiffness, damping])
        if restitution_coeff is None:
            self.shape_restitution_coeff.append(self.DEFAULT_SHAPE_RESTITUTION_COEFF)
        else:
            self.shape_restitution_coeff.append(restitution_coeff)

        mesh_geom = fcl.BVHModel()
        mesh_geom.beginModel(len(mesh.vertices), len(mesh.faces))
        mesh_geom.addSubModel(mesh.vertices, mesh.faces)
        mesh_geom.endModel()
        self.shape_fcl_geoms.append(mesh_geom)

        self.shape_meshes.append(mesh)
        self.shape_mass.append(mesh.mass * density)
        self.shape_inv_mass.append(1.0 / self.shape_mass[-1])
        self.shape_inertia.append(mesh.moment_inertia * density)
        self.shape_inv_inertia.append(np.linalg.inv(self.shape_inertia[-1]))

    def add_shape_cylinder(
        self,
        pos: Vec3,
        radius: float,
        lz: float,  # length along z axis
        orient: Vec4,  # Initial orientation of the cylinder in quaternion
        stiffness: float | None = None,
        damping: float | None = None,
        restitution_coeff: float | None = None,
        fixed: bool = False,
        density: float = 1.0,
    ):
        """Adds a cylinder shape to the model.

        Args:
            pos: The position of the cylinder
            radius: The radius of the cylinder
            lz: The length of the cylinder along z axis
            orient: The initial orientation of the cylinder in quaternion
            stiffness: The stiffness of the cylinder
            damping: The damping of the cylinder
            restitution_coeff: The restitution coefficient of the cylinder
            fixed: Whether the cylinder is fixed

        NOTE:
            The orientation is given in quaternion (w, x, y, z) format (scalar first).
            When quaternion is [1, 0, 0, 0], the cylinder is aligned with the z axis.
        """
        self.shape_type.append(GeoType.CYLINDER)
        if not isinstance(pos, np.ndarray):
            pos = np.asarray(pos, dtype=np.float64)
        if pos.shape != (3,):
            raise RuntimeError("pos must have a shape of (3,)")

        if radius <= 0.0:
            raise RuntimeError("radius must be positive")
        if lz <= 0.0:
            raise RuntimeError("lz must be positive")

        if not isinstance(orient, np.ndarray):
            orient = np.asarray(orient, dtype=np.float64)
        if orient.shape != (4,):
            raise RuntimeError("orient must have a shape of (4,)")
        ll = np.linalg.norm(orient)  # normalize the orientation
        if ll < 1e-8:
            raise RuntimeError("cylinder orient is too small, must be non-zero")
        orient /= ll

        self.shape_flags.append(ShapeFlags.ACTIVE.value if not fixed else 0)
        trans = np.eye(4)
        trans[:3, :3] = R.from_quat(orient, scalar_first=True).as_matrix()
        trans[:3, 3] = pos
        self.shape_transform.append(trans)
        self.shape_inv_transform.append(np.linalg.inv(trans))

        if stiffness is None:
            stiffness = self.DEFAULT_SHAPE_PENALTY_STIFFNESS
        elif stiffness <= 0.0:
            raise RuntimeError(f"Invalid stiffness ({stiffness} must > 0)")
        if damping is None:
            damping = self.DEFAULT_SHAPE_PENALTY_DAMPING
        elif damping < 0.0:
            raise RuntimeError(f"Invalid damping ({damping} must >= 0)")
        self.shape_penalty_params.append([stiffness, damping])
        if restitution_coeff is None:
            self.shape_restitution_coeff.append(self.DEFAULT_SHAPE_RESTITUTION_COEFF)
        else:
            self.shape_restitution_coeff.append(restitution_coeff)
        self.shape_fcl_geoms.append(fcl.Cylinder(radius, lz))

        self.shape_meshes.append(None)
        self.shape_mass.append(np.pi * radius**2 * lz * density)
        self.shape_inv_mass.append(1.0 / self.shape_mass[-1])
        self.shape_inertia.append(inertia.cylinder_inertia(self.shape_mass[-1], radius, lz))
        self.shape_inv_inertia.append(np.linalg.inv(self.shape_inertia[-1]))

    def add_shape_box(
        self,
        pos: Vec3,
        size: Vec3,  # length along x, y, z axes
        orient: Vec4,  # Initial orientation of the box in quaternion
        stiffness: float | None = None,
        damping: float | None = None,
        restitution_coeff: float | None = None,
        fixed: bool = False,
        density: float = 1.0,
    ):
        """Adds a box shape to the model.

        Args:
            pos: The position of the box
            size: The size of the box, i.e., length along x, y, z axes
            orient: The initial orientation of the box in quaternion
            stiffness: The stiffness of the box
            damping: The damping of the box
            restitution_coeff: The restitution coefficient of the box
            fixed: Whether the box is fixed

        NOTE:
            The orientation is given in quaternion (w, x, y, z) format (scalar first).
        """
        self.shape_type.append(GeoType.BOX)
        if not isinstance(pos, np.ndarray):
            pos = np.asarray(pos, dtype=np.float64)
        if pos.shape != (3,):
            raise RuntimeError("pos must have a shape of (3,)")

        if not isinstance(size, np.ndarray):
            size = np.asarray(size, dtype=np.float64)
        if size.shape != (3,):
            raise RuntimeError("size must have a shape of (3,)")

        if not isinstance(orient, np.ndarray):
            orient = np.asarray(orient, dtype=np.float64)
        if orient.shape != (4,):
            raise RuntimeError("orient must have a shape of (4,)")
        ll = np.linalg.norm(orient)  # normalize the orientation
        if ll < 1e-8:
            raise RuntimeError("cylinder orient is too small, must be non-zero")
        orient /= ll

        self.shape_flags.append(ShapeFlags.ACTIVE.value if not fixed else 0)
        trans = np.eye(4)
        trans[:3, :3] = R.from_quat(orient, scalar_first=True).as_matrix()
        trans[:3, 3] = pos
        self.shape_transform.append(trans)
        self.shape_inv_transform.append(np.linalg.inv(trans))

        if stiffness is None:
            stiffness = self.DEFAULT_SHAPE_PENALTY_STIFFNESS
        elif stiffness <= 0.0:
            raise RuntimeError(f"Invalid stiffness ({stiffness} must > 0)")
        if damping is None:
            damping = self.DEFAULT_SHAPE_PENALTY_DAMPING
        elif damping < 0.0:
            raise RuntimeError(f"Invalid damping ({damping} must >= 0)")

        self.shape_penalty_params.append([stiffness, damping])
        if restitution_coeff is None:
            self.shape_restitution_coeff.append(self.DEFAULT_SHAPE_RESTITUTION_COEFF)
        else:
            self.shape_restitution_coeff.append(restitution_coeff)
        self.shape_fcl_geoms.append(fcl.Box(size[0], size[1], size[2]))

        self.shape_meshes.append(None)
        self.shape_mass.append(size[0] * size[1] * size[2] * density)
        self.shape_inv_mass.append(1.0 / self.shape_mass[-1])
        size_sqr = size**2
        diag = np.asarray([size_sqr[1] + size_sqr[2], size_sqr[0] + size_sqr[2], size_sqr[0] + size_sqr[1]]) * (
            self.shape_mass[-1] / 12.0
        )
        self.shape_inertia.append(np.diag(diag))
        self.shape_inv_inertia.append(np.linalg.inv(self.shape_inertia[-1]))

    def add_shape_sphere(
        self,
        pos: Vec3,
        radius: float,
        stiffness: float | None = None,
        damping: float | None = None,
        restitution_coeff: float | None = None,
        fixed: bool = False,
        density: float = 1.0,
    ):
        """Adds a sphere shape to the model.

        Args:
            pos: The position of the sphere
            radius: The radius of the sphere
            stiffness: The stiffness of the sphere
            damping: The damping of the sphere
            restitution_coeff: The restitution coefficient of the sphere
        """
        self.shape_type.append(GeoType.SPHERE)
        if not isinstance(pos, np.ndarray):
            pos = np.asarray(pos, dtype=np.float64)
        if pos.shape != (3,):
            raise RuntimeError("pos must have a shape of (3,)")

        trans = np.eye(4)
        trans[:3, 3] = pos
        self.shape_transform.append(trans)
        inv_trans = np.eye(4)
        inv_trans[:3, 3] = -pos
        self.shape_inv_transform.append(inv_trans)

        self.shape_flags.append(ShapeFlags.ACTIVE.value if not fixed else 0)
        if stiffness is None:
            stiffness = self.DEFAULT_SHAPE_PENALTY_STIFFNESS
        elif stiffness <= 0.0:
            raise RuntimeError(f"Invalid stiffness ({stiffness} must > 0)")
        if damping is None:
            damping = self.DEFAULT_SHAPE_PENALTY_DAMPING
        elif damping < 0.0:
            raise RuntimeError(f"Invalid damping ({damping} must >= 0)")

        self.shape_penalty_params.append([stiffness, damping])
        if restitution_coeff is None:
            self.shape_restitution_coeff.append(self.DEFAULT_SHAPE_RESTITUTION_COEFF)
        else:
            self.shape_restitution_coeff.append(restitution_coeff)
        self.shape_fcl_geoms.append(fcl.Sphere(radius))

        self.shape_meshes.append(None)
        self.shape_mass.append(4.0 * np.pi * (radius**3) * density / 3.0)  # mass of the sphere
        self.shape_inv_mass.append(1.0 / self.shape_mass[-1])
        self.shape_inertia.append((2.0 / 5.0) * self.shape_mass[-1] * radius**2 * np.eye(3))
        self.shape_inv_inertia.append(np.linalg.inv(self.shape_inertia[-1]))

    def add_cloth_mesh(
        self,
        mesh: Trimesh,
        pos: Vec3,
        orient: Vec4,
        density: float,
        ks: float | None = None,  # triangle stretch stiffness
        ksd: float | None = None,  # triangle stretch damping
        kr: float | None = None,  # triangle shear stiffness
        krd: float | None = None,  # triangle shear damping
        kb: float | None = None,  # triangle bending stiffness
        kbd: float | None = None,  # triangle bending damping
        fixed: list[int] | None = None,  # indices of fixed cloth particles
    ):
        """Adds a cloth mesh shape to the model.

        Args:
            mesh: The mesh of the cloth
            pos: The initial position of the cloth
            orient: The initial orientation of the cloth (as a quaternion)
            density: Density of the cloth material
            ks: The stretch stiffness of the cloth
            ksd: The stretch damping of the cloth
            kr: The shear stiffness of the cloth
            krd: The shear damping of the cloth
            kb: The bending stiffness of the cloth
            kbd: The bending damping of the cloth
            fixed: The indices of the fixed cloth particles
        """
        if not isinstance(pos, np.ndarray):
            pos = np.asarray(pos, dtype=np.float64)
        if pos.shape != (3,):
            raise RuntimeError("pos must have a shape of (3,)")

        if not isinstance(orient, np.ndarray):
            orient = np.asarray(orient, dtype=np.float64)
        if orient.shape != (4,):
            raise RuntimeError("orient must have a shape of (4,)")
        ll = np.linalg.norm(orient)  # normalize the orientation
        if ll < 1e-8:
            raise RuntimeError("cylinder orient is too small, must be non-zero")
        orient /= ll

        # uv coordinates of the mesh: always take y-coordinate out as the UV coordiante
        if not np.all(mesh.vertices[..., 1] == 0.0):
            raise RuntimeError("mesh vertices must be in the x-z plane to extract UV coordinates")
        uv = np.column_stack((mesh.vertices[..., 0], mesh.vertices[..., 2]))

        trans = np.eye(4)
        trans[:3, :3] = R.from_quat(orient, scalar_first=True).as_matrix()
        trans[:3, 3] = pos
        mesh.apply_transform(trans)

        if density <= 0.0:
            raise RuntimeError("density must be positive")

        n_already = self.particle_count
        N = mesh.vertices.shape[0]
        self.particle_q.extend(mesh.vertices.tolist())
        self.particle_qd.extend([[0.0, 0.0, 0.0]] * N)

        particle_mass = [0.0] * N
        for i, t in enumerate(mesh.faces):
            mass = mesh.area_faces[i] * density / 3.0
            particle_mass[t[0]] += mass
            particle_mass[t[1]] += mass
            particle_mass[t[2]] += mass
        self.particle_mass.extend(particle_mass)

        self.particle_radius.extend([self.DEFAULT_PARTICLE_RADIUS] * N)

        flags = [ParticleFlags.ACTIVE.value] * N
        if fixed is not None:
            for f in fixed:
                flags[f] = 0
        self.particle_flags.extend(flags)

        self.particle_drag.extend([0.0] * N)
        self.particle_restitution_coeff.extend([self.DEFAULT_PARTICLE_RESTITUTION_COEFF] * N)
        self.particle_uv.extend(uv.tolist())

        if ks is None:
            ks = self.DEFAULT_CLOTH_STRETCH_STIFFNESS
        elif ks <= 0.0:
            raise RuntimeError(f"Invalid stretch stiffness ({ks} must > 0)")
        if ksd is None:
            ksd = self.DEFAULT_CLOTH_STRETCH_DAMPING
        elif ksd < 0.0:
            raise RuntimeError(f"Invalid stretch damping ({ksd} must >= 0)")
        if kr is None:
            kr = self.DEFAULT_CLOTH_SHEAR_STIFFNESS
        elif kr <= 0.0:
            raise RuntimeError(f"Invalid shear stiffness ({kr} must > 0)")
        if krd is None:
            krd = self.DEFAULT_CLOTH_SHEAR_DAMPING
        elif krd < 0.0:
            raise RuntimeError(f"Invalid shear damping ({krd} must >= 0)")

        faces = mesh.faces + n_already
        self.tri_indices.extend(faces.tolist())
        self.tri_areas.extend(mesh.area_faces.tolist())
        self.tri_materials.extend([[ks, ksd, kr, krd]] * mesh.faces.shape[0])

        # add bending edges
        edge_map = {}
        for t0, t1, t2 in faces:
            if (t1, t0) in edge_map:
                v0 = edge_map[(t1, t0)]
                # t1 -> t0 is central edge
                # v0 is on the left side of the edge
                # t2 is on the left side of t0 -> t1
                self.edge_indices.append([t0, t1, t2, v0])
                self.edge_rest_angle.append(0.0)
                self.edge_materials.append([kb, kbd])
            else:
                edge_map[(t0, t1)] = t2

            if (t2, t1) in edge_map:
                v0 = edge_map[(t2, t1)]
                self.edge_indices.append([t1, t2, t0, v0])
                self.edge_rest_angle.append(0.0)
                self.edge_materials.append([kb, kbd])
            else:
                edge_map[(t1, t2)] = t0

            if (t0, t2) in edge_map:
                v0 = edge_map[(t0, t2)]
                self.edge_indices.append([t2, t0, t1, v0])
                self.edge_rest_angle.append(0.0)
                self.edge_materials.append([kb, kbd])
            else:
                edge_map[(t2, t0)] = t1

    def add_shape_half_space_plane(
        self,
        plane_normal: Vec3,
        plane_offset: float,
        stiffness: float | None = None,
        damping: float | None = None,
        restitution_coeff: float | None = None,
    ):
        """Adds a half space plane shape to the model.

        Args:
            plane_normal: The normal direction of the plane
            plane_offset: The offset of the plane

        NOTE:
            A halfspace plane is always fixed in the world frame.
        """
        self.shape_type.append(GeoType.HALF_SPACE_PLANE)

        if not isinstance(plane_normal, np.ndarray):
            plane_normal = np.asarray(plane_normal, dtype=np.float64)
        if plane_normal.shape != (3,):
            raise RuntimeError("plane_normal must have a shape of (3,)")
        nrm_len = np.linalg.norm(plane_normal)
        if nrm_len < 1e-8:
            raise RuntimeError(f"Plane normal ({plane_normal}) is too small")
        plane_normal = plane_normal / nrm_len

        # construct the transform matrix that transforms the plane to the world frame
        rot, _ = R.align_vectors(plane_normal, np.array([0.0, 1.0, 0.0], dtype=np.float64))
        trans = np.eye(4)
        trans[:3, :3] = rot.as_matrix()
        trans[:3, 3] = np.array(plane_normal * plane_offset, dtype=np.float64)

        self.shape_flags.append(0)  # NOT active (Fixed)
        self.shape_transform.append(trans)
        self.shape_inv_transform.append(np.linalg.inv(trans))
        if stiffness is None:
            stiffness = self.DEFAULT_SHAPE_PENALTY_STIFFNESS
        elif stiffness <= 0.0:
            raise RuntimeError(f"Invalid stiffness ({stiffness} must > 0)")

        if damping is None:
            damping = self.DEFAULT_SHAPE_PENALTY_DAMPING
        elif damping < 0.0:
            raise RuntimeError(f"Invalid damping ({damping} must >= 0)")

        self.shape_penalty_params.append([stiffness, damping])
        if restitution_coeff is None:
            self.shape_restitution_coeff.append(self.DEFAULT_SHAPE_RESTITUTION_COEFF)
        else:
            self.shape_restitution_coeff.append(restitution_coeff)
        self.shape_fcl_geoms.append(fcl.Halfspace(plane_normal, plane_offset))

        self.shape_mass.append(0.0)
        self.shape_inv_mass.append(0.0)
        self.shape_inertia.append(np.zeros((3, 3)))
        self.shape_inv_inertia.append(np.zeros((3, 3)))

    def add_gravitational(self, i: int, j: int, G: float):
        """Adds a gravitational attraction force between two particles in the system

        Args:
            i: The index of the first particle
            j: The index of the second particle
            G: The gravitational constant
        """
        self.gravitational_pairs.append(i)
        self.gravitational_pairs.append(j)
        self.gravitational_constant.append(G)

    def finalize(self) -> Model:
        """
        Finalize the builder and create a concrete Model for simulation.

        This method transfers all simulation data from the builder to a Model instance,
        returning a Model object ready for simulation. It should be called after all
        elements (particles, bodies, shapes, joints, etc.) have been added to the builder.

        Returns:
            Model: A fully constructed Model object containing all simulation data.

        NOTES:
            - This method also perform necessary validation of simulation setup
        """
        m = Model()
        m.gravity = np.array(self.up_axis.to_vector(), dtype=np.float64) * self.gravity
        m.up_axis = self.up_axis

        # ---------------------
        # particles

        m.particle_count = self.particle_count
        if m.particle_count > 0:
            m.particle_q = np.asarray(self.particle_q, dtype=np.float64)
            m.particle_qd = np.asarray(self.particle_qd, dtype=np.float64)
            # check all mass are positive
            for c in self.particle_mass:
                if c < 1e-8:
                    raise RuntimeError(f"Particle mass ({c}) is too small")
            m.particle_mass = np.asarray(self.particle_mass, dtype=np.float64)
            m.particle_inv_mass = np.reciprocal(m.particle_mass)
            m.particle_radius = np.asarray(self.particle_radius, dtype=np.float64)
            m.particle_flags = np.asarray(self.particle_flags, dtype=np.int32)
            m.particle_drag = np.asarray(self.particle_drag, dtype=np.float64)
            m.particle_restitution_coeff = np.asarray(self.particle_restitution_coeff, dtype=np.float64)
            m.particle_uv = np.asarray(self.particle_uv, dtype=np.float64)
            # For fixed particles, ensure:
            #   1) the velocity to be zero
            #   2) the inverse mass to be zero
            for i in range(self.particle_count):
                if m.particle_flags[i] & ParticleFlags.ACTIVE.value == 0:
                    m.particle_qd[i] = np.zeros(3)
                    m.particle_inv_mass[i] = 0.0
                if m.particle_restitution_coeff[i] < 0.0 or m.particle_restitution_coeff[i] > 1.0:
                    raise RuntimeError(
                        f"Particle restitution coefficient ({m.particle_restitution_coeff[i]}) must be in [0.0, 1.0]"
                    )

        # ---------------------
        # springs
        for i in self.spring_indices:
            if i < 0 or i >= self.particle_count:
                raise RuntimeError(f"Spring particle index ({i}) is out of range")
        for ke, kd in zip(self.spring_stiffness, self.spring_damping, strict=True):
            if ke < 0 or kd < 0:
                raise RuntimeError(f"Failed to satisfy (stiffness >= 0) and (damping >= 0): ke={ke}, kd={kd}")
        m.spring_indices = np.array(self.spring_indices, dtype=np.int32).reshape((-1, 2))
        m.spring_stiffness = np.array(self.spring_stiffness, dtype=np.float64)
        m.spring_damping = np.array(self.spring_damping, dtype=np.float64)
        for i in range(self.spring_count):
            r = self.spring_rest_length[i]
            if r < 0.0:
                r = np.linalg.norm(m.particle_q[m.spring_indices[i, 0]] - m.particle_q[m.spring_indices[i, 1]])
            self.spring_rest_length[i] = r
        m.spring_rest_length = np.array(self.spring_rest_length, dtype=np.float64)

        # ---------------------
        # gravitational
        for i in self.gravitational_pairs:
            if i < 0 or i >= self.particle_count:
                raise RuntimeError(f"Gravitational particle index ({i}) is out of range")
        for G in self.gravitational_constant:
            if G < 0:
                raise RuntimeError(f"Gravitational constant ({G}) is negative")
        m.gravitational_pairs = np.array(self.gravitational_pairs, dtype=np.int32).reshape((-1, 2))
        m.gravitational_constant = np.array(self.gravitational_constant, dtype=np.float64)

        # ---------------------
        # mesh triangles
        if len(self.tri_indices) > 0:
            m.tri_indices = np.asarray(self.tri_indices, dtype=np.int32)
            m.tri_areas = np.asarray(self.tri_areas, dtype=np.float64)
            m.tri_materials = np.asarray(self.tri_materials, dtype=np.float64)

            m.tri_uv_inv = np.empty((m.tri_indices.shape[0], 2, 2), dtype=np.float64)
            for i in range(m.tri_indices.shape[0]):
                t0, t1, t2 = m.tri_indices[i]
                m.tri_uv_inv[i] = np.linalg.inv(
                    np.column_stack((m.particle_uv[t1] - m.particle_uv[t0], m.particle_uv[t2] - m.particle_uv[t0]))
                )

            m.edge_indices = np.asarray(self.edge_indices, dtype=np.int32)
            m.edge_rest_angle = np.asarray(self.edge_rest_angle, dtype=np.float64)
            m.edge_materials = np.asarray(self.edge_materials, dtype=np.float64)

        # ---------------------
        # shapes
        if len(self.shape_transform) > 0:
            m.shape_type = self.shape_type
            m.shape_transform = np.stack(self.shape_transform, axis=0)
            m.shape_flags = np.asarray(self.shape_flags, dtype=np.int32)
            m.shape_inv_transform = np.stack(self.shape_inv_transform, axis=0)
            m.shape_penalty_params = np.asarray(self.shape_penalty_params, dtype=np.float64)
            m.shape_restitution_coeff = np.asarray(self.shape_restitution_coeff, dtype=np.float64)
            # check all restitution coefficients are in [0.0, 1.0]
            for i in range(m.shape_count):
                if m.shape_restitution_coeff[i] < 0.0 or m.shape_restitution_coeff[i] > 1.0:
                    raise RuntimeError(
                        f"Shape restitution coefficient[{i}] ({m.shape_restitution_coeff[i]}) must be in [0.0, 1.0]"
                    )
            m.shape_fcl_geoms = self.shape_fcl_geoms
            m.shape_meshes = self.shape_meshes
            m.shape_mass = np.asarray(self.shape_mass, dtype=np.float64)
            m.shape_inv_mass = np.asarray(self.shape_inv_mass, dtype=np.float64)
            m.shape_inertia = np.stack(self.shape_inertia, axis=0)
            m.shape_inv_inertia = np.stack(self.shape_inv_inertia, axis=0)

        # finally, initialize the collision objects
        m.initialize_collision_objects()
        return m
