...
def eval_cloth_bending_forces(model: Model, state: State) -> None:
    ...
    for ii in range(model.edge_count):
        ...
        n1 = np.cross(e, state.particle_q[v2] - state.particle_q[v0])
        n2 = np.cross(state.particle_q[v3] - state.particle_q[v0], e)
        ...
        # the contraint
        C_theta = np.arctan2(sin_theta, cos_theta)
        dot_C_theta = 0.0

        dx2 = -n1 * (e_nrm / n1_nrm**2)
        dot_C_theta += np.dot(dx2, state.particle_qd[v2])
        dx3 = -n2 * (e_nrm / n2_nrm**2)
        dot_C_theta += np.dot(dx3, state.particle_qd[v3])

        dx0 = -dx2 * (np.dot(state.particle_q[v1] - state.particle_q[v2], e) / (e_nrm**2)) - dx3 * (
            np.dot(state.particle_q[v1] - state.particle_q[v3], e) / (e_nrm**2)
        )
        dot_C_theta += np.dot(dx0, state.particle_qd[v0])
        dx1 = dx2 * (np.dot(state.particle_q[v0] - state.particle_q[v2], e) / (e_nrm**2)) + dx3 * (
            np.dot(state.particle_q[v0] - state.particle_q[v3], e) / (e_nrm**2)
        )
        dot_C_theta += np.dot(dx1, state.particle_qd[v1])
        kb, kbd = model.edge_materials[ii]

        kb_C = C_theta * kb + dot_C_theta * kbd
        state.particle_f[v0] -= dx0 * kb_C
        state.particle_f[v1] -= dx1 * kb_C
        state.particle_f[v2] -= dx2 * kb_C
        state.particle_f[v3] -= dx3 * kb_C
