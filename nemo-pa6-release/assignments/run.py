...
class Runner_PA_6:
    def __init__(self, model: Model, solver: SolverBase):
        ...
        # ps.look_at((0.5, 0.5, 1.), (0.5, 0.5, 0.))  # camera from above, looking at origin

        print(self.state_0.fluid_particle_q)
        self.pc = ps.register_point_cloud("fluid particles", self.state_0.fluid_particle_q)
        self.pc.set_radius(model.fluid_cell_size / 3, relative=False)
    ...
    def launch(self):
        ...
        while not ps.window_requests_close():
            ...
            if self.running:
                ...
                self.state_0, self.state_1 = self.state_1, self.state_0
                # update the viewer states for rendering
                self.pc.update_point_positions(self.state_0.fluid_particle_q)
...
def launch_pa_6(model: Model, solver: SolverBase, plspec: PlotSpec | None = None):
    runner = Runner_PA_6(model, solver)
...
# Entry point for PA6
@app.command("pa6", help="Run PA6 simulation")
def pa6(config: Annotated[str, typer.Argument(help="Scene configuration file")]):
    module = importlib.import_module(".pa6", package=__package__)
    # 1. Load configuration and create model
    model, solver = module.load_scene(config)
    launch_pa_6(model, solver)
